...
private:
	...
	//1个小字2个大字
	bool CheckComposeOneSmallTwoBig(const CardGroup cards, const int &value) const
	{
		return cards[0][value] >= 1 && cards[1][value] >= 2;
	}

	//2个小字1个大字
	bool CheckComposeTowSmallOneBig(const CardGroup cards, const int &value) const
	{
	...
	//一壹%, 二贰%， 三叁%, ... 绞牌
	bool CheckComposeLaiZiJiaoPai(const CardGroup cards, const int &value) const
	{
		if (laizi_num() == 0) return false;
	...
	/************************************2癞子************************************/
	//2%% 2710
	bool CheckCompose2710TowLaiziAnd2(const int &value) const
	{
		return laizi_num() >= 2 && value == 2;
	}

	//%7% 2710
	bool CheckCompose2710TowLaiziAnd7(const int &value) const
	{
		return laizi_num() >= 2 && value == 7;
	}

	//%%10 2710
	bool CheckCompose2710TowLaiziAnd10(const int &value) const
	{
		return laizi_num() >= 2 && value == 10;
	}

	//1%% 123
	bool CheckCompose123TowLaiziAnd1(const int &value) const
	{
		return laizi_num() >= 2 && value == 1;
	}

	//%2% 123 
	bool CheckCompose123TowLaiziAnd2(const int &value) const
	{
		return laizi_num() >= 2 && value == 2;
	}

	//%%3 123
	bool CheckCompose123TowLaiziAnd3(const int &value) const
	{
